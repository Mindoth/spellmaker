...
public class ForceSigilItem extends AbstractSigilItem {
    ...
    public void effectOnAllEntitiesInList(Entity source, Entity directSource, Entity target, List<Integer> stats, DimVec3 location) {
        ...
        if ( target == directSource ) direction = directSource.getLookAngle();
        else if ( directSource instanceof AbstractSpellEntity spell && !spell.isMultiTarget() ) direction = directSource.getDeltaMovement().normalize();
